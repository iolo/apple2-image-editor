...
const refreshColorPreview = () => {
  elements.fgPreview.style.background = state.fg;
  elements.bgPreview.style.background = state.bg;
};

const isHgrMode = (modeId) => modeId === 'hgrColor' || modeId === 'hgrMono';
const isDhgrMode = (modeId) => modeId === 'dhgrColor' || modeId === 'dhgrMono';
const getModeXScale = () =>
  typeof state.mode.xscale === 'number' ? state.mode.xscale : 1;
...
const setMode = (modeId, width, height) => {
  ...
  state.pixels = createBlankPixels(state.width, state.height, '#000');
  state.fg = '#fff';
  state.bg = '#000';
  state.undo = [];
  state.redo = [];
...
const setupToolbar = () => {
  ...
  document
    ...
    .addEventListener('click', () => ensureDialog(elements.settingsDialog));
  document
    .getElementById('aboutButton')
    .addEventListener('click', () => ensureDialog(elements.aboutDialog));
